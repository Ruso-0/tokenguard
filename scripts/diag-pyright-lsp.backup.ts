...
async function main() {
  ...
  const timer = setTimeout(() => {
    console.error('\n[TIMEOUT] 15s elapsed — pyright did not respond in time.');
    proc.kill('SIGTERM');
    process.exit(2);
  }, TIMEOUT_MS);

  // Spawn pyright-langserver
  const proc: ChildProcess = spawn('pyright-langserver', ['--stdio'], {
  ...
  // ── Step 4: Request codeActions for each error diagnostic ───────
  ...
  for (let i = 0; i < errorDiags.length; i++) {
    ...
    if (codeActionResult?.__lspError) {
      ...
      console.log(`  ⚠ EMPTY — No code actions returned.`);
    } else {
      console.log(`  ✓ ${codeActionResult.length} code action(s) returned:`);
  ...
  // ── Step 5: Also try executeCommand for pyright.addMissingImport ─
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  ...
  // ── Step 6: Try codeAction without "only" filter ────────────────
  if (errorDiags.length > 0) {
    ...
    if (unfilteredResult?.__lspError) {
      ...
      console.log(`  ⚠ EMPTY — No code actions returned (even without filter).`);
    } else {
      console.log(`  ✓ ${unfilteredResult.length} code action(s) returned:`);
